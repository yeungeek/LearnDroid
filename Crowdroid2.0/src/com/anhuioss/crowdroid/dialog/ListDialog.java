...
public class ListDialog extends Dialog implements OnItemClickListener,
		ServiceConnection, OnItemLongClickListener {
		...
		public void requestCompleted(String service, int type,
			...
			if (statusCode != null && statusCode.equals("200")) {
				...
				if (type == CommHandler.TYPE_GET_MY_LISTS) {
					...
					listInfoListData.get(selectedItemPosition).setFollow(
							selectedItemFollowStatus);
					createListView(listInfoListData);
	...
	public void onStart() {
		...
		setTitle(String.format(mContext.getString(R.string.list_dialog),
				screenName));
			...
			try {
				...
				for (ListInfo listInfo : listInfoList) {
					...
					map.put("name", listInfo.getName()
							+ ((listInfo.getFollow() != null && listInfo
									.getFollow().equals("true")) ? "(Follow)"
									: "(Unfollow)"));
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						try {
							apiServiceInterface.request(
									...
									CommHandler.TYPE_SET_FOLLOW_LIST,
