...
public class SohuOAuthConstant {
	...
	private String getBaseString() throws UnsupportedEncodingException {
		...
		if ((oauthToken != null) && (oauthToken.length() > 0)) {
			bsss = bsss + "&oauth_token="
					+ URLEncoder.encode(oauthToken, this.encoding);
		}
		if ((oauthVerifier != null) && (oauthVerifier.length() > 0)) {
			bsss = bsss + "&oauth_verifier="
					+ URLEncoder.encode(oauthVerifier, this.encoding);
		}
	...
	public String getOAuthSignature(String data) throws Exception {
		...
		try {
			...
			String oauthKey = URLEncoder.encode(consumerSecret, this.encoding)
					+ "&"
					+ (((oauthTokenSecret == null) || (oauthTokenSecret
							.equals(""))) ? "" : URLEncoder.encode(
							oauthTokenSecret, this.encoding));
	...
	public void separateTokenAndSecret(String sTokenAndSecret) {
		if (sTokenAndSecret.contains("&") && sTokenAndSecret.contains("=")) {
	...
	public String getAuthorizUrl() {
		...
		String token = "oauth_token=" + oauthToken;
		String url = "http://api.t.sohu.com/oauth/authorize?" + token
				+ "&oauth_callback=oob&clientType=phone";

		return url;
	}

	public void setOAuthVerifier(String oauthVerifier) {
		SohuOAuthConstant.oauthVerifier = oauthVerifier;
	}
	...
	public void setOAuthToken(String oauthToken) {
		SohuOAuthConstant.oauthToken = oauthToken;
	}
	...
	public void setOAuthTokenSecret(String oauthTokenSecret) {
		SohuOAuthConstant.oauthTokenSecret = oauthTokenSecret;
	}
