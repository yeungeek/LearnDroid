...
//		}
		...
		parameter.add(new QParameter("latitude", map.get("latitude")
				.toString().substring(0, 9)));
		// parameter.add(new QParameter("longitude", map.get("longitude")
		// .toString()));
		// parameter
...
//		parameter.add(new QParameter("pagesize", "1"));
		parameter.add(new QParameter("pageinfo", "1"));
	...
	public static CommResult updateTags(Map map) {
		...
		result.setResponseCode(TencentParserHandler.parseResponseCode(res));
		result.setMessage(res);
		if (result.getMessage().contains("errcode")) {
			try {
				JSONObject error = new JSONObject(result.getMessage());
				if (error.getString("error_code").equals("360002")) {
					result.setResponseCode("360002");
				} else if (error.getString("error_code").equals("360004")) {
					result.setResponseCode("360004");
				}
	...
	public static synchronized CommResult getAreaTimeline(Map map) {
		...
		parameters.add(new QParameter("province", map.get("stateCode")
				.toString()));
