...
import com.anhuioss.crowdroid.HotFollowRetweetTimelineActivity;
...
import com.anhuioss.crowdroid.cfb.AlbumsActivity;
import com.anhuioss.crowdroid.cfb.AlbumsActivity;
...
import com.anhuioss.crowdroid.map.MyLocationActivity;
...
public class DiscoveryActivity extends BasicActivity implements
	...
	private int[] imageIds = { R.drawable.discovery_user_timeline_icon,
			...
			R.drawable.discovery_lbs_icon, R.drawable.discovery_hot_users_icon,
			...
			R.drawable.discovery_tag_icon, R.drawable.discovery_mood_icon,
			R.drawable.discovery_favitore_timeline_icon,

			R.drawable.discovery_mood_icon, R.drawable.photoicon,
			R.drawable.docicon, R.drawable.discovervedio };
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String[] appNames = { getString(R.string.user_timeline),
				getString(R.string.search), getString(R.string.trends),
				...
				getString(R.string.wangyi_rss),
				getString(R.string.photoCenter),
				getString(R.string.docCentert), getString(R.string.vedioCenter) };
		...
		if (statusData.getCurrentService().equals(IGeneral.SERVICE_NAME_WANGYI)) {
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("ItemImage", imageIds[14]);
			map.put("ItemText", appNames[14]);
			listImageItem.add(map);
		}
		// 图片中心
		if (statusData.getCurrentService().equals(
				IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("ItemImage", imageIds[15]);
			map.put("ItemText", appNames[15]);
			listImageItem.add(map);
		}
		// 文档中心
		if (statusData.getCurrentService().equals(
				IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("ItemImage", imageIds[16]);
			map.put("ItemText", appNames[16]);
			listImageItem.add(map);
		}
		// 视频中心
		if (statusData.getCurrentService().equals(
				IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("ItemImage", imageIds[17]);
			map.put("ItemText", appNames[17]);
			listImageItem.add(map);
		}
		...
		public void onItemClick(AdapterView<?> arg0, View view, int position,
			...
			case 1: {
				if (statusData.getCurrentService().equals(
						IGeneral.SERVICE_NAME_SOHU)) {
					// user search
					openSohuSearchManager();

				} else if (statusData.getCurrentService().endsWith(
						IGeneral.SERVICE_NAME_SINA)) {
					openSinaSearchManager();
				} else {
					openSearchManager();
				}
			...
			if (getString(R.string.trends).equals(item.get("ItemText"))) {
				...
				if (statusData.getCurrentService().equals(
					...
					// getString(R.string.casual_watch_setting_public_tip),
			...
			} else if (getString(R.string.retweets_mention_me).equals(
					item.get("ItemText"))) {
				...
				// mood
				openMoodManager();
			} else if (getString(R.string.favorite_timeline).equals(
					item.get("ItemText"))) {
				// favorite
				Intent i = new Intent(DiscoveryActivity.this,
				...
				i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				startActivity(i);
			} else if (getString(R.string.wangyi_rss).equals(
					item.get("ItemText"))) {
				// rss
				if (statusData.getCurrentService().equals(
						IGeneral.SERVICE_NAME_WANGYI)) {

					new WangyiRsslistDialog(DiscoveryActivity.this).show();
				}

			} else if ((getString(R.string.photoCenter)).equals(item
					.get("ItemText"))) {
				// rss
				if (statusData.getCurrentService().equals(
						IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
			...
			} else if ((getString(R.string.docCentert)).equals(item
					.get("ItemText"))) {
				// rss
				if (statusData.getCurrentService().equals(
						IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
			...
			} else if ((getString(R.string.vedioCenter)).equals(item
					.get("ItemText"))) {
				// rss
				if (statusData.getCurrentService().equals(
						IGeneral.SERVICE_NAME_CROWDROID_FOR_BUSINESS)) {
	...
	public void onClick(View v) {
		...
		case R.id.head_refresh: {
			...
			Intent home = new Intent(DiscoveryActivity.this,
	...
	private void openSohuSearchManager() {
	...
	private void openSinaSearchManager() {
		String[] items = getResources().getStringArray(
				R.array.discovery_sina_search_manager);
	...
	private void openCasualWatchManager() {
		...
		if (restore.contains("Casual_Watch_Type")
			...
			if (!radioPreStr.equals("")) {
				...
				case 2: {
					Intent searchIntent = new Intent(DiscoveryActivity.this,
							KeywordSearchActivity.class);
	...
	// casual watch
	private void openCasualWatchManagerDialog() {
		...
		if (isFirstIn == true) {
			...
			editor.commit();
			AlertDialog dialog = new AlertDialog.Builder(DiscoveryActivity.this)
					.setTitle(R.string.setting_casual_watch_choose)
					.setMessage(R.string.casual_watch_setting_tip)
	...
	private void WangyiHotretweet() {
		CharSequence[] choose = getResources().getStringArray(
				R.array.wangyi_hotretweet_choose);
		AlertDialog dialog = new AlertDialog.Builder(DiscoveryActivity.this)
				.setItems(choose, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						case 0: {
							...
							String cancal = getResources().getString(
									R.string.cancel);
							String ok = getResources().getString(R.string.ok);
							CharSequence[] Timechoose = getResources()
									.getStringArray(
											R.array.wangyi_hotretweet_timechoose);
							// AlertDidlog dialogtop=new
							// AlertDialog.Builder(DiscoveryActivity.this)
							// setTheme();
							new AlertDialog.Builder(DiscoveryActivity.this)
									.setTitle(
											R.string.wangyi_hot_retweet_timechoose)
									.setIcon(R.drawable.insert_emotion)
						...
						case 1: {
							Intent intent = new Intent(DiscoveryActivity.this,
									HotFollowRetweetTimelineActivity.class);
							intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
							startActivity(intent);
							break;
						}
	...
	private void openHotUserManager() {
		if (statusData.getCurrentService().equals(IGeneral.SERVICE_NAME_WANGYI)) {
			// hot user
			Intent intent = new Intent(DiscoveryActivity.this,
					HotUsersActivity.class);
