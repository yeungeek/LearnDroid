...
public class FollowedActivity extends BasicActivity implements OnClickListener,
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (position + 1 == data.size()
				...
				if (nextCursor != 0
					...
					if (IGeneral.SERVICE_NAME_WANGYI.equals(statusData
							.getCurrentService())) {
						nextCursor = nextCursor + 20;
					}
					refresh();
				}
			} else {
	...
	public void onStart() {
		...
		} else if (bundle.containsKey("context")
				&& bundle.getString("context").equals("NotificationService")) {// 后台粉丝通知跳转
