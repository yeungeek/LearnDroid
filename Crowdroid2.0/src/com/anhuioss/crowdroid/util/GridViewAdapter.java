...
public class GridViewAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		final String image_path = list.get(position).get("itemImage");
				...
				if (event.getAction() == actionFalg) {
					...
					if (commType == CommHandler.TYPE_GET_ALBUM_PHOTOS) {
						...
						intent = new Intent(mContext,
								PreviewAlbumPhotosActivity.class);
						...
						bundle.putSerializable("timelineInfoList",
								timelineInfoList);
						...
						if (mContext instanceof SendMessageActivity) {
							...
							// intent.putExtras(bundle);
							// mContext.startActivity(intent);
							SendMessageActivity.updateText.append(list.get(
									position).get("itemText"));
							alertDialog.dismiss();
						...
						// update lbs message
							...
							SharedPreferences.Editor editor = sharedPreferences
									.edit();
							editor.putString("SHARE_INIT_UPDATE_LBS_STATUS",
									list.get(position).get("itemText"));
							editor.putBoolean(
									"WHEATHER_FROM_LBS_OUTSIDE_INSERT", true);
							editor.commit();
							alertDialog.dismiss();
						}
						// retweet message
						...
						// direct message
							...
							editor.commit();
							alertDialog.dismiss();
						} else if (mContext instanceof LBSdetailActivity) {
							if (alertDialog == null) {
								String url = image_path.split("\n")[1];
								...
								if (previewHtmlData.length() > 0) {
									Intent intentx = new Intent(mContext,
											PreviewImageActivity.class);
									Bundle bundle = new Bundle();
									bundle.putString("url",
											previewHtmlData.toString());
								...
								LBSdetailActivity.updateText.append(list.get(
										position).get("itemText"));
								alertDialog.dismiss();
							}
						}
