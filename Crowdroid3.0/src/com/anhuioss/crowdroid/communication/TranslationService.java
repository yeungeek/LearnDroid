...
public class TranslationService extends Service {
	...
	private TranslationServiceInterface.Stub translationServiceInterface = new TranslationServiceInterface.Stub() {
		...
		public void request(String engine, long type,
				TranslationServiceListener listener, Map map) throws RemoteException {
	...
	private void connect(final String engine, final long type,
		...
		Thread th = new Thread(new Runnable() {
			...
			public void run() {
				...
				CommResult result = translationHandler.request(engine, type, map);
				...
				mHandler.post(new Runnable() {
					public void run() {
						try {
							...
							listener.requestCompleted(engine, messageId != null ? Long.valueOf(messageId) : type,
									statusCode, message);
