...
public class TipsDialog extends Dialog implements OnClickListener {
	...
	private ArrayList<String> getTipsData(Context context) {
		...
		try {
			...
			InputStreamReader inputReader = new InputStreamReader(context.getResources().getAssets().open(tipsFileName));
					...
					if (xmlPullParser.getName().equals("tip")) {
						...
						while (xmlPullParser.next() != XmlPullParser.END_DOCUMENT) {
							if (xmlPullParser.getEventType() == XmlPullParser.START_TAG) {
								...
								// Min-Version
								if (xmlPullParser.getName().equals("min-version")) {
									xmlPullParser.next();
								...
								// Max-Version
								if (xmlPullParser.getName().equals("max-version")) {
									xmlPullParser.next();
