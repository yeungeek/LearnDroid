...
public class AddListDialog extends Dialog implements OnItemClickListener, ServiceConnection {
					...
					if(statusCode != null && statusCode.equals("200")){
						...
						try {
							apiServiceInterface.request(statusData.getCurrentService(),
									CommHandler.TYPE_GET_FOLLOW_LISTS, apiServiceListener,
									parameters);
	...
	public void onStart() {
		...
		// Set Title
		setTitle(String.format(mContext.getString(R.string.user_list), screenName));
		...
		if(listInfoListData.size() > 0){
			...
			if(isExist){
				Toast.makeText(mContext 
							, String.format(mContext.getResources().getString(R.string.is_exist), list.getName())
	...
	private void createListView(ArrayList<ListInfo> listInfoList) {
		...
		for (ListInfo listInfo : listInfoList) {
			...
			map.put("name", listInfo.getName() + "(" + listInfo.getUserInfo().getScreenName() + ")");
