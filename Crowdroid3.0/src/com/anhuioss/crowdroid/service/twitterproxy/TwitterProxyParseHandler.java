	...
	public static ArrayList<TimeLineInfo> parseTimeline(String message) {
	    ...
	    try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				timeLineInfo.setStatus(status.getString("text").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replace("〜", "~").replaceAll("\\r|\\n|\\t|\\s", " "));
				timeLineInfo.setMessageId(status.getString("id"));
				timeLineInfo.setinReplyToStatusId(status.getString("in_reply_to_status_id"));
...
ArrayList<TimeLineInfo> directMessageInfoList = new ArrayList<TimeLineInfo>();
	    ...
	    try {
			...
			for (int i = 0; i < statuses.length(); i++) {
				...
				directMessageInfo.setStatus(status.getString("text").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replace("〜", "~"));
	...
	public static ArrayList<TimeLineInfo> parseSearchInfo(String message) {
		...
		try {
			...
			for (int i = 0; i < searchinfototal.length(); i++) {
				...
				timeLineInfo.setTime(searchinfosingle.getString("created_at").replace(",", ""));
				timeLineInfo.setStatus(searchinfosingle.getString("text").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("&quot;", "\""));
	...
	public static ArrayList<TimeLineInfo> parseRetweetBy(String message) {
	    ...
	    try {
			...
			for (int i = 0; i < statuses.length(); i++) {
				...
				timeLineInfo.setFavorite(status.getString("favorited"));
				timeLineInfo.setTime(status.getString("created_at"));
				timeLineInfo.setStatus(status.getString("text").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replace("〜", "~"));
				timeLineInfo.setMessageId(status.getString("id"));
				timeLineInfo.setinReplyToStatusId(status.getString("in_reply_to_status_id"));
				...
				JSONObject retweetedStatus = status.getJSONObject("retweeted_status");
